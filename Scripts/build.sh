...
test "$(uname -m)" = arm64
swift build --scratch-path .build/openblue --configuration "$configuration"
binary_dir="$(swift build --scratch-path .build/openblue --configuration "$configuration" --show-bin-path)"
